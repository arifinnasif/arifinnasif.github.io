...
* I use thumbnails, so I can upload arbitrary sized images but then only display small ones. Drop the full-size original in `_originals/` (`_originals/tn/images/` for project images, `_originals/images/` for the profile photo), run `./_optimize_images.sh`, and it writes the web-sized WebP plus a same-format fallback into `tn/` and `images/`. The html template serves the WebP via `<picture>` and falls back to the original format. 
